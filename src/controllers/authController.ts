...
const setUserCookie = (res: Response, photon_id: string, name: string, role: string = 'student') => {
...
const getUserFromCookie = (req: Request) => {
...
export const showFacultyLogin = (req: Request, res: Response) => {
    const redirect = req.query.redirect as string || '';
...
export const handleFacultyLogin = async (req: Request, res: Response) => {
    const { faculty_id, password, redirect } = req.body;
    
    try {
        // Authenticate faculty against database
        const user = await authenticateUser(faculty_id, password);
        ...
        if (user) {
            ...
            setUserCookie(res, user.photon_id as string, user.name as string, 'faculty');
...
export const handleLogin = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const user = await authenticateUser(photon_id, password);
        ...
        if (user) {
            ...
            // Set cookie to indicate user is logged in with role
            setUserCookie(res, user.photon_id as string, user.name as string, 'student');
...
export const handleRegister = async (req: Request, res: Response) => {
    ...
    if (!photon_id || !name || !phone || !studentClass || !course || !email || !password || !confirmPassword) {
        const missingFields = [];
        if (!photon_id) missingFields.push('photon_id');
    ...
    try {
        ...
        const result = await registerUser({ photon_id, name, phone, class: studentClass, course, email, password });
...
// Export getUserFromCookie function
